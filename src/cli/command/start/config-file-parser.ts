...
function parseCluster(config: any): ICluster {
    ...
    const servers: { [id: string]: IEndpoint } = parseClusterServers(config['servers']);
...
function parseLogger(config: any): logger.ILogger {
    ...
    const level: logger.Level = config && config['level']
        ? parseLogLevel(config['level'])
        : 'error';

    const machineReadable: boolean = config && config['machineReadable']
