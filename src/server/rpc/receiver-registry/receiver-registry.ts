...
export class RpcReceiverRegistry implements IRpcReceiverRegistry {
  private appendEntriesRequestReceivers: Set<
    IRpcReceiver<'append-entries', 'request'>
  >;
  private appendEntriesResponseReceivers: Set<
    IRpcReceiver<'append-entries', 'response'>
  >;
  private requestVoteRequestReceivers: Set<
    IRpcReceiver<'request-vote', 'request'>
  >;
  private requestVoteResponseReceivers: Set<
    IRpcReceiver<'request-vote', 'response'>
  >;
  ...
  >(receiver: IRpcReceiver<RpcProcedureTypeMap[P], RpcCallTypeMap[C]>): void {
  ...
  >(
    ...
    type _ReturnType = Set<
      IRpcReceiver<RpcProcedureTypeMap[P], RpcCallTypeMap[C]>
    >;
