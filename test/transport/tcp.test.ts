...
describe('tcp transport', function() {
    ...
    describe('a tcp transport', function() {
        describe('#send', function() {
            context('when there is not a server listening at the send destination', function() {
                const transport = createTcpTransport();

                describe('the transport', function() {
                    it('gets an ECONNREFUSED', function(done) {
                        transport.send(endpoint0, data.hello).then(function() {
                            done('expected an ECONNREFUSED but instead sent data was received');
                        }, function(err) {
            ...
            context('when there is a server listening at the send destination', function() {
                const transport = createTcpTransport(),
                    connections: IConnectionRegistry<net.Socket> = createConnectionRegistry<net.Socket>(),
            ...
            context('when there is a server listening at two destinations', function() {
                const transport = createTcpTransport(),
                      ...
                      connections1: IConnectionRegistry<net.Socket> = createConnectionRegistry<net.Socket>(),
        ...
        describe('#listen', function() {
            ...
            it('binds the server instance to the provided host and port', function(done) {
                ...
                anotherServer.listen(endpoint0.port, endpoint0.host, function() {
                    done(`expected server to be bound to ${endpoint0.host}:${endpoint0.port}`);
                });
            });
        });
